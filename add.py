#!/usr/bin/env python3
"""
Quick add artwork to Siyu's gallery.

Usage:
  python3 add.py /path/to/file.pdf
  python3 add.py /path/to/photo.jpg
  python3 add.py /path/to/folder/   # add all PDFs/images in folder
...
Images are saved in two versions:
  images/artworks/siyu/           ← watermarked (pushed to GitHub)
  images/artworks/siyu/_original/ ← clean originals (local only, gitignored)
"""
...
def process_file(filepath, out_dir, orig_dir, title=None, desc=None):
...
def main():
    parser = argparse.ArgumentParser(description="Add artwork to Siyu's gallery")
    ...
    out_dir = os.path.join(IMG_BASE, CHILD)
    ...
    if os.path.isdir(path):
        ...
        for f in files:
            ...
            new_entries.extend(process_file(f, out_dir, orig_dir, desc=args.desc))
    elif os.path.isfile(path):
        print(f"Processing: {os.path.basename(path)}")
        new_entries = process_file(path, out_dir, orig_dir, title=args.title, desc=args.desc)
    else:
    ...
    print(f"\n✅ Added {len(new_entries)} artwork(s) for Siyu")
    ...
    if args.push:
        ...
        os.chdir(SCRIPT_DIR)
        os.system("git add .")
        os.system(f'git commit -m "Add {len(new_entries)} artwork(s) for Siyu"')
        os.system("git push")
